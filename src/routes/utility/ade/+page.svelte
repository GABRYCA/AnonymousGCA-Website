<script>
    ...
    function convertBitsToBytes(bits) {
        if (bits % 8 !== 0) {
            return '<p class="text-danger">Inserire un multiplo di 8</p>';
        }
        return bits / 8;
    }
    ...
    function handleBitSubmit() {
        ...
        byteResult = `<p class='mt-2 fs-3 text-info'>${bytes} Byte - 2<sup class='text-warning'>${power}</sup> (${parola} ${power} Bit)</p>`;
    }
</script>
